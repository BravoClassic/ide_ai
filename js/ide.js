...
async function renderMarkdown(text) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "Content-Type": "text/plain", // GitHub API expects raw text, not JSON
    },
    body: markdownText,
  });
...
$(document).ready(async function () {
  ...
  require(["vs/editor/editor.main"], function (ignorable) {
    ...
    layout.registerComponent("ai-chat", function (container, state) {
      ...
      chatContainer.innerHTML = `
        <div style="display: flex; flex-direction: column; margin-top: 10px;">
            <textarea id="ai-chat-input" placeholder="Ask AI about the code..." 
                   style="width: 100%; bac height: 80px; resize: vertical;
                          padding: 8px; background-color: #333333;border-radius: 5px; color:#ffffff;"></textarea>
            <button id="ai-chat-submit" 
                    style="margin-top: 5px; padding: 8px; border: none; background-color: #007bff; 
    ...
    <div class="chat-message user-message">
        ...
        inputField.value = "";
        aiResponse = await chatWithAI(message);

        chatMessages.innerHTML += `<div> ${await renderMarkdown(
          aiResponse
        )}</div>`;
